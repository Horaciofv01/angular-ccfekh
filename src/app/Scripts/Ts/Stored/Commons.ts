...
export class Commons
{
    ...
    ZoomByWidth()
    {
      ...
      //Zoom match to 1900 = 1; to 1200 = 0.65
      const mDiference : number = 0.0004978663;
      ...
      if(windowWidth > 1900)
        ...
        {     
          let mZoom = 1 - ((1900 - windowWidth) * mDiference);


            $('body').css({
                '-moz-transform': 'scale(' + mZoom.toFixed(2) + ')',
                'zoom': + mZoom.toFixed(2)});        
...
{
    mValues: strKeyValues[] = [];

    constructor ()
    {
        this.mValues.push( new strKeyValues( 'לולאות הרצליה', 'hertzlia.json', null, null ) );
        this.mValues.push( new strKeyValues( '2 לולאות דוגמה', 'loops2.json', null, null ) );
